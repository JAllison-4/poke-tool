...
import { Autocomplete, Button, Card, TextField, Typography, Box, Chip } from "@mui/material";
...
export default function PokemonSearch() {
  ...
  async function handleSelect(event: any, value: string | null) {
    ...
    setPokemon(null);
    if (value) {
      const poke = await fetchPokemon(value.toLowerCase());
      setPokemon(poke);
    }
  }
    ...
    <Box className="random-pokemon-root">
      <Autocomplete
        ...
        renderInput={(params) => (
          ...
          variant="outlined"
          sx={{
            input: { color: "#fff", background: "#23262F" },
            label: { color: "#ccc" },
            "& .MuiOutlinedInput-root": {
            "& fieldset": { borderColor: "#555" },
            "&:hover fieldset, &.Mui-focused fieldset": { borderColor: "#ef5350" },
            },
          }}
          />
        )}
